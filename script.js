...
// Removed import of Sortable.js as per the plan
...
function playSound(name) {
    // Resume audio context if suspended (common browser autoplay policy)
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const materialMissionPayoutInput = document.getElementById('materialMissionPayout');

  // Create Ship Modal elements
  ...
  // Clear platform capacity input if value is 1 on focus
  ...
  const translations = {
    es: {
      ...
      no_completed_missions: "No hay misiones completadas en el historial.",
    ...
    en: {
      ...
      no_completed_missions: "No completed missions in history.",
  ...
  const translatePage = (lang) => {
    document.querySelectorAll('[data-key]').forEach(element => {
      ...
      if (translations[lang][key]) {
        // Exclude specific elements from direct textContent assignment if they contain child elements
        // This prevents overwriting icons or other nested HTML structure
        ...
        // Check if element is itself one of the excluded buttons, or if its parent is.
        // This aims to prevent double-translation or breaking existing HTML structures within buttons.
        ...
        if (!isExcludedButton && element.children.length === 0) {
          ...
          // Find the actual text node to update, leave other child elements (like icons) intact
          const textNode = Array.from(element.childNodes).find(node => node.nodeType === Node.TEXT_NODE && node.textContent.trim().length > 0);
          if (textNode) {
            textNode.textContent = " " + translations[lang][key]; // Prepend space for consistency
          } else if (element.textContent.trim() === '') { // Fallback if no specific text node but element has no other content
            element.textContent = translations[lang][key];
          }
        }
      }
  ...
  const renderHistoryView = () => {
    ...
    if (missionHistory.length === 0) {
      historyTableBody.innerHTML = `<tr><td colspan="4" class="text-center p-4 text-muted-glow">${t.no_completed_missions}</td></tr>`;
    }
  ...
  const showAddMaterialForm = (missionId, missionName) => {
    ...
    materialMissionPayoutInput.value = ''; 
  ...
  const showEditMaterialForm = (cargo, missionId, missionName) => {
    ...
    materialMissionPayoutInput.value = mission ? (mission.payout || 0) : 0; 
  ...
  materialForm.addEventListener('submit', (e) => {
    ...
    const newMissionPayout = parseInt(materialMissionPayoutInput.value, 10) || 0;
    missions[missionIndex].payout = newMissionPayout;
  ...
  clearHistoryBtn.addEventListener('click', () => {
    const t = translations[currentLang];
    if (confirm(t.clear_history + '? ' + 'Esta acción no se puede deshacer.')) {
      missionHistory = [];
      saveMissionHistory();
      renderTable();
      playSound('delete'); 
    ...
    ships.forEach(ship => {
        ...
        if (Array.isArray(ship.platforms)) {
            ship.platforms.forEach(platform => {
                ...
                let loadedItemsHtml = '<ul class="list-unstyled mb-0">';
                if (Array.isArray(platform.loadedContainers) && platform.loadedContainers.length > 0) {
                    platform.loadedContainers.forEach(lc => {
                        const { mission, cargo } = findCargoAndMission(lc.cargoId);
                        const materialName = cargo ? cargo.material : 'Unknown';
                        const missionName = mission ? mission.name : 'Unknown';
                        loadedItemsHtml += `
                            <li class="d-flex justify-content-between align-items-center loaded-cargo-item p-1">
                                <span class="small"><i class="bi bi-box-seam me-2"></i>${materialName} (${lc.size} SCU) <small class="text-muted-glow">- ${missionName}</small></span>
                                <button class="btn btn-sm btn-danger unload-single-container-btn" data-ship-id="${ship.id}" data-platform-id="${platform.id}" data-container-id="${lc.id}" data-cargo-id="${lc.cargoId}" title="${t.unload_cargo}"><i class="bi bi-x-lg"></i></button>
                            </li>
                        `;
                    });
                } else {
                    loadedItemsHtml += `<li class="text-muted-glow small p-2">${t.no_cargo_on_platform}</li>`;
                }
                loadedItemsHtml += '</ul>';
        ...
        shipCard.innerHTML = `
            <div class="d-flex justify-content-between align-items-center mb-3">
                <h4 class="title-font mb-0"><i class="bi bi-rocket-takeoff me-2"></i>${ship.name}</h4>
                <div class="btn-group">
...
// Event listener for image modal to load correct image
if (imageModalEl) {
